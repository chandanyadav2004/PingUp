            ...
            <div className=' rounded-lg h-96 flex items-center justify-center relative' style={{backgroundColor: background}}>
...

                {
                    ...
                    <div className='flex mt-4 gap-2'>
                        {
                            bgColor.map((color) => (
                                <button  key={color} className=' h-6 w-6 rounded-full ring cursor-pointer ' style={{backgroundColor: color}} onClick={() => setBackground(color)} />
                            ))
                        }
                    </div>
                    ...
                    <div className=' flex mt-4 gap-2'>
                        <button onClick={() => {setMode('text'); setMedia(null); setPreviewUrl(null);}} className={` flex-1 flex items-center justify-center gap-2 p-2 rounded cursor-pointer ${mode === 'text' ? 'bg-white text-black' : 'bg-zinc-800 '}`}>
                            <TextIcon className='' size={18} />Text
                        </button>
                        ...
                        <label className={`flex-1 flex items-center justify-center gap-2 p-2 rounded cursor-pointer ${mode === 'media' ? 'bg-white text-black' : 'bg-zinc-800 '}`}>
                            <input type="file" onChange={(e) => {handeMediaUpload(e); setMode('media')}} className=' hidden' accept='image/*, video/*' />
                            <Upload size={18}  /> Photo/Video
                        </label>
                    </div>

                    <button onClick={() => toast.promise(handeCreateStory(), {
                        loading: 'Saving...',
