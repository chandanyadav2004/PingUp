...
const StoriesBar = () => {
  ...
  const fetchStories = async () => {
    ...
    if (data.success) {
      setStories(data.stories); // adjust key if backend sends something different
    } else {
      toast.error(data.message || "Failed to load stories");
    }
  } catch (error) {
    console.error(error);
    toast.error(error.message || "Error fetching stories");
  }
  };
      ...
      <div className=" flex gap-4 pb-5">
        ...
        {/* Story Card  */}
        {
            stories.map((story,index) => (
                <div key={index} onClick={() => setViewStory(story)} className={`relative rounded-lg shadow min-w-30 max-w-30 max-h-40 cursor-pointer hover:shadow-lg transition-all duration-200 bg-gradient-to-b from-indigo-500 to-purple-600 hover:from-indigo-700 hover:to-purple-800 active:scale-95`}>
                        ...
                        <div className=" absolute inset-0 z-1 rounded-lg bg-black overflow-hidden">
                          {
                            ...
                            <img src={story.media_url} alt="image and video url " className=" h-full w-full object-cover hover:scale-110 transition duration-500 opacity-70 hover:opacity-80 " /> 
                            : 
